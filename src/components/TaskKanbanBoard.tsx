...
export const TaskKanbanBoard: React.FC = () => {
  ...
  const [aiTaskInsights, setAiTaskInsights] = useState<Record<string, string>>({});
  ...
  const generateTaskInsights = async () => {
    try {
      // Create mock contacts from task data for AI analysis
      const taskContacts = tasks.map(task => ({
        id: task.id,
        name: task.assignedUserName || 'Unassigned',
        firstName: task.assignedUserName?.split(' ')[0] || 'User',
        lastName: task.assignedUserName?.split(' ').slice(1).join(' ') || '',
        email: `${task.assignedUserName?.toLowerCase().replace(' ', '.')}@company.com` || 'user@company.com',
        title: 'Team Member',
        company: 'Internal',
        industry: 'Technology',
      ...
      // Generate smart suggestions for task optimization
      const insights = await generateInsights(taskContacts);
      setSmartSuggestions(insights.slice(0, 3));
          ...
          {smartSuggestions.length > 0 && (
            <div className="mt-2 flex items-center space-x-2">
              ...
              <span className="text-sm text-purple-700 font-medium">
                {smartSuggestions.length} AI optimization suggestions available
              </span>
            </div>
          )}
        </div>
