...
export const ContactsModal: React.FC<ContactsModalProps> = ({ isOpen, onClose }) => {
  ...
  const [aiFilterMode, setAiFilterMode] = useState<'all' | 'high-score' | 'needs-attention' | 'opportunities'>('all');
          ...
          <div className="flex items-center justify-between p-6 border-b border-gray-200 bg-gray-50">
            <div className="flex items-center space-x-4 flex-1">
              ...
              <div className="relative">
                <button
                  onClick={() => {}} 
                  className="flex items-center space-x-2 px-4 py-2 bg-gradient-to-r from-purple-50 to-blue-50 border border-purple-200 rounded-lg text-purple-700 hover:from-purple-100 hover:to-blue-100 transition-colors"
                >
                  <Brain className="w-4 h-4" />
                ...
                <div className="absolute bottom-full left-0 mb-2 w-56 bg-white border border-gray-200 rounded-lg shadow-xl z-10">
                  {[
                    ...
                    return (
                      <button
                        key={filter.value}
                        onClick={() => setAiFilterMode(filter.value as any)}
                        className={`w-full text-left px-4 py-2 hover:bg-gray-50 transition-colors first:rounded-t-lg last:rounded-b-lg flex items-center space-x-2 ${
                          aiFilterMode === filter.value ? 'bg-blue-50 text-blue-700 font-medium' : 'text-gray-700'
                        }`}
                      >
